...
function get_cache($sparql){
	$dir = "cache/";
	$file = $dir.md5($sparql).".cache";
...
function store_cache($sparql, $data){
	$dir = "cache/";
	$file = $dir.md5($sparql).".cache";
...
class sparql_connection
{
	...
	function dispatchQuery( $sparql )
	{
		...
		curl_setopt($ch, CURLOPT_USERPWD, "omr-client-upm:omr.client.upm.2012");
	...
	function parse()
	{
	  ...
	  // Now we can pass the contents, maybe if you want
			...
			foreach ($lines as $val) {
				...
				//TODO Hay un error de parseo por el punto y coma de los datos
				$data = str_replace("&amp;", "And", $data);
				$data = str_replace("&", "And", $data);
				$data = str_replace(";", "PC", $data);
